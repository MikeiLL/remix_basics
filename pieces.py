...
def pre_tatum(track):
    ...
    for segment in track.segments:
        ...
        and (segment.start < track.tatums[0].start):
            ...
            segs_in += 1
        if ((segment.start < track.tatums[0].start) and (segment.tatum != None)):
            print("* Segment {} - {} \n\t overlaps with {}".format(segment.start, \
                                                                segment.end, segment.tatum))
    print("*", segs_in, "viable pre-tatum segment(s):")
    print("* \t from: {} to {}.".format(start[0].start, start[-1].end))
    print("* First tatum starts at {}.".format(track.tatums[0].start))
